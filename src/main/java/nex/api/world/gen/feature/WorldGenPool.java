...
public class WorldGenPool extends WorldGenerator
{
    ...
    public WorldGenPool(Block fluidBlockIn, IBlockState surroundingBlockIn)
    {
        fluidBlock = fluidBlockIn;
                ...
                for(int l2 = 0; l2 < 16; ++l2)
                {
                    for(int k = 0; k < 8; ++k)
                    {
                        ...
                        if(flag)
                        {
                            ...
                            if(k < 4 && !material.isSolid() && world.getBlockState(pos.add(k1, k, l2)).getBlock() != fluidBlock)
                            {
                                return false;
                            }
                        }
            ...
            for(int l1 = 0; l1 < 16; ++l1)
            {
                for(int i3 = 0; i3 < 16; ++i3)
                {
                    for(int i4 = 0; i4 < 8; ++i4)
                    {
                        ...
                        {
                            world.setBlockState(pos.add(l1, i4, i3), i4 >= 4 ? Blocks.AIR.getDefaultState() : fluidBlock.getDefaultState(), 2);
                        }
                    }
                }
            }
