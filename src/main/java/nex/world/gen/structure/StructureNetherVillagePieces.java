...
public class StructureNetherVillagePieces
{
    public static void registerVillagePieces()
    {
        MapGenStructureIO.registerStructureComponent(House1.class, "nex:ViBH");
        MapGenStructureIO.registerStructureComponent(Field1.class, "nex:ViDF");
        MapGenStructureIO.registerStructureComponent(Field2.class, "nex:ViF");
        MapGenStructureIO.registerStructureComponent(Torch.class, "nex:VViL");
        MapGenStructureIO.registerStructureComponent(Hall.class, "Vnex:ViPH");
        MapGenStructureIO.registerStructureComponent(House4Garden.class, "nex:VViSH");
        MapGenStructureIO.registerStructureComponent(WoodHut.class, "nex:VViSmH");
        MapGenStructureIO.registerStructureComponent(Church.class, "Vinex:VST");
        MapGenStructureIO.registerStructureComponent(House2.class, "Vinex:VS");
        MapGenStructureIO.registerStructureComponent(Start.class, "ViSnex:Vtart");
        MapGenStructureIO.registerStructureComponent(Path.class, "ViSRnex:V");
        MapGenStructureIO.registerStructureComponent(House3.class, "Vinex:VTRH");
        MapGenStructureIO.registerStructureComponent(Well.class, "Vinex:VW");
    }
    ...
    public abstract static class Village extends StructureComponent
    {
        ...
        protected int getAverageGroundLevel(World worldIn, StructureBoundingBox structurebb)
        {
            ...
            for(int k = this.boundingBox.minZ; k <= this.boundingBox.maxZ; ++k)
            {
                ...
                {
                    blockpos$mutableblockpos.setPos(l, 32, k);

                    if(structurebb.isVecInside(blockpos$mutableblockpos))
                    {
                        i += Math.max(WorldGenUtil.getSolidBlockBelow(worldIn, blockpos$mutableblockpos, 80).getY(), worldIn.getSeaLevel() + 1);
                        ++j;
                    }
                }
            }
        ...
        protected void spawnVillagers(World worldIn, StructureBoundingBox structurebb, int x, int y, int z, int count)
        {
            if(this.villagersSpawned < count)
            {
                for(int i = this.villagersSpawned; i < count; ++i)
                {
                    ...
                    {
                        EntityPigZombie pigZombie = new EntityPigZombie(worldIn);
                        pigZombie.setLocationAndAngles((double) j + 0.5D, (double) k, (double) l + 0.5D, 0.0F, 0.0F);
                        pigZombie.onInitialSpawn(worldIn.getDifficultyForLocation(new BlockPos(pigZombie)), null);
                        pigZombie.enablePersistence();
                        worldIn.spawnEntity(pigZombie);
                    }
                    else
                    {
                        EntityPigtificate pigtificate = new EntityPigtificate(worldIn);
                        pigtificate.setLocationAndAngles((double) j + 0.5D, (double) k, (double) l + 0.5D, 0.0F, 0.0F);
                        worldIn.spawnEntity(pigtificate);
                    }
                }
            }
        }
        ...
        protected IBlockState getBiomeSpecificBlockState(IBlockState blockstateIn)
        {
            ...
            else if(this.structureType == 3)
            {
                ...
                if(blockstateIn.getBlock() == Blocks.NETHER_BRICK)
                {
                    return Blocks.NETHER_BRICK.getDefaultState().withProperty(BlockPlanks.VARIANT, BlockPlanks.EnumType.SPRUCE);
                }
                ...
                if(blockstateIn.getBlock() == Blocks.NETHER_BRICK_FENCE)
                {
            ...
            else if(this.structureType == 2)
            {
                ...
                if(blockstateIn.getBlock() == Blocks.NETHER_BRICK)
                {
                    return Blocks.NETHER_BRICK.getDefaultState().withProperty(BlockPlanks.VARIANT, BlockPlanks.EnumType.ACACIA);
                }
