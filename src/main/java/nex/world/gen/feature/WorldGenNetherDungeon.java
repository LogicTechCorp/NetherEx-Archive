...
public class WorldGenNetherDungeon extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if(j2 >= 1 && j2 <= 5)
        {
                ...
                for(int i4 = 3; i4 >= -1; --i4)
                {
                    for(int k4 = l1; k4 <= i2; ++k4)
                    {
                        ...
                        else if(worldIn.getBlockState(blockpos1).getMaterial().isSolid() && worldIn.getBlockState(blockpos1).getBlock() != Blocks.CHEST)
                        {
                            ...
                            {
                                worldIn.setBlockState(blockpos1, set.getWallBlockState(), 2);
                            }
                        }
                    }
                }
            ...
            {
                NetherEx.logger.error("Failed to fetch mob spawner entity at (" + position.getX() + ", " + position.getY() + ", " + position.getZ() + ")");
            }
