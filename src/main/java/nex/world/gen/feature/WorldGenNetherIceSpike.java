...
public class WorldGenNetherIceSpike extends WorldGenerator
{
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if(worldIn.getBlockState(position).getBlock() == ModContent.Blocks.netherrack.getStateFromMeta(0).getBlock())
        {

            for(int p = 0; p < 24; p++)
            {
                IBlockState state = worldIn.getBlockState(position.add(0, p, 0));
                ...
                for(int i1 = -l; i1 <= l; ++i1)
                {
                    ...
                    for(int j1 = -l; j1 <= l; ++j1)
                    {
                        ...
                        if((i1 == 0 && j1 == 0 || f1 * f1 + f2 * f2 <= f * f) && (i1 != -l && i1 != l && j1 != -l && j1 != l || rand.nextFloat() <= 0.75F))
                        {
                            ...
                            if(iblockstate.getBlock().isAir(iblockstate, worldIn, position.add(i1, k, j1)) || block == Blocks.NETHERRACK || block == Blocks.SOUL_SAND || block == Blocks.GRAVEL)
                            {
