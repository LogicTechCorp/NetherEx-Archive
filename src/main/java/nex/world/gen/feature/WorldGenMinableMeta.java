...
public class WorldGenMinableMeta extends WorldGenerator
{
    ...
    public WorldGenMinableMeta(IBlockState state, int blockCount, Block block, int meta)
    {
        this.oreBlock = state;
        this.numberOfBlocks = blockCount;
        this.predicate = BlockMatcher.forBlock(block);
        this.meta = meta;
    }
                        ...
                        if (d12 * d12 + d13 * d13 < 1.0D)
                        {
                            for (int j2 = l; j2 <= k1; ++j2)
                            {
                                ...
                                if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D)
                                {
                                    ...
                                    if (state.getBlock().isReplaceableOreGen(state, worldIn, blockpos, this.predicate) && state.getBlock().getMetaFromState(state) == this.meta)
                                    {
