...
public class ExplosionSpore extends Explosion
{
    ...
    public ExplosionSpore(World worldIn, Entity entityIn, double x, double y, double z, float size, List<BlockPos> affectedPositions)
    {
        this(worldIn, entityIn, x, y, z, size, false, true, affectedPositions);
    }

    @SideOnly(Side.CLIENT)
    public ExplosionSpore(World worldIn, Entity entityIn, double x, double y, double z, float size, boolean flaming, boolean smoking, List<BlockPos> affectedPositions)
    {
    ...
    {
        world.playSound(null, explosionX, explosionY, explosionZ, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 4.0F, (1.0F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.2F) * 0.7F);

        if(explosionSize >= 2.0F && isSmoking)
        {
            NetherEx.proxy.spawnParticle(world, explosionX, explosionY, explosionZ, 1.0D, 0.0D, 0.0D, new ParticleSporeExplosionHuge.Factory());
        }
        else
        {
            NetherEx.proxy.spawnParticle(world, explosionX, explosionY, explosionZ, 1.0D, 0.0D, 0.0D, new ParticleSporeExplosionLarge.Factory());
        }
