...
public class BlockEnokiCap extends BlockBase
{
        ...
        else
        {
            ...
            if(world.isAirBlock(blockpos) && blockpos.getY() > 0)
            {
                ...
                if(i < 5 && rand.nextInt(1) == 0)
                {
                    ...
                    if(block == Blocks.NETHERRACK || block == NEXBlocks.netherrack)
                    {
                        flag = true;
                    }
                    else if(block == NEXBlocks.enokiStem)
                    {
                        int j = 1;
                        ...
                        for(int k = 0; k < 4; ++k)
                        {
                            ...
                            if(block1 != NEXBlocks.enokiStem)
                            {
                                if(block1 == Blocks.NETHERRACK || block1 == NEXBlocks.netherrack)
                                {
                                    flag1 = true;
                                }
                    ...
                    {
                        world.setBlockState(pos, NEXBlocks.enokiStem.getDefaultState(), 2);
    ...
    private boolean canSurvive(World world, BlockPos pos)
    {
        ...
        if(block != NEXBlocks.enokiStem && (block != Blocks.NETHERRACK || block != NEXBlocks.netherrack))
        {
            if(block == Blocks.AIR)
            {
                int i = 0;
    ...
    {
        world.setBlockState(pos.down(), NEXBlocks.enokiStem.getDefaultState(), 2);
    ...
    private static void growTreeRecursive(World world, BlockPos pos, Random rand, BlockPos pos1, int x, int z)
    {
        ...
        for(int j = 0; j < i; ++j)
        {
            ...
            world.setBlockState(blockpos, NEXBlocks.enokiStem.getDefaultState(), 2);
        }
        ...
        if(z < 4)
        {
            ...
            for(int k = 0; k < l; ++k)
            {
                ...
                {
                    flag = true;
                    world.setBlockState(blockpos1, NEXBlocks.enokiStem.getDefaultState(), 2);
        ...
        if(!flag)
        {
            world.setBlockState(pos.down(i), NEXBlocks.enokiCap.getDefaultState().withProperty(AGE, 5), 2);
        }
    }
